...
2. If the component is not on the page yet:
   ...
   - In left panel under *Custom Components*, drag **`digiField360 AI Natural Language Query Hub`** onto the page.
...
3. Now test the search bar on your screen:
   ...
   * **Test 1D (One-Click Quick Pills)**:
     ...
     - Click any column header (e.g. *Annual Revenue*) ➡️ verify table sorts ASC/DESC.
...
3. Find an unassigned Work Order (e.g. *"Generator coolant leak"*).
...
5. **Verify Outcome**:
   ...
   - The field `AI_Assignment_Reason__c` displays the reason:
     > *"Assigned based on Generator skills match, 92.5% first-time fix rate, and closest proximity (4.2 km)."*
