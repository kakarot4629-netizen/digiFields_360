...
async function runApiTests() {
  ...
  // 2. Auth Login
  await test('POST /api/auth/login returns token and technician profile', async () => {
  ...
  await test('GET /api/sync/morning-payload returns complete offline dataset', async () => {
    const res = await fetch(`${BASE_URL}/api/sync/morning-payload?technicianId=TECH-001`, {
      headers: { 'x-mock-mode': 'true' }
    });
    const data = await res.json();
    if (!data.success || !Array.isArray(data.workOrders) || !Array.isArray(data.equipmentHistory) || !Array.isArray(data.knowledgeArticles)) {
  ...
  await test('POST /api/ai/query resolves Accounts query dynamically', async () => {
    ...
    const data = await res.json();
    if (!data.success || data.targetObject !== 'Account' || !data.soqlGenerated.includes('Account') || !data.aiSummary || !data.records?.length) {
  ...
  await test('POST /api/ai/query resolves Technicians query dynamically', async () => {
    ...
    const data = await res.json();
    if (!data.success || data.targetObject !== 'Technician__c' || !data.soqlGenerated.includes('Technician__c') || !data.records?.length) {
      throw new Error(`NL2SOQL Technician query failed: ${JSON.stringify(data)}`);
    }
  });
  ...
  await test('POST /api/ai/query resolves Maintenance Alerts dynamically', async () => {
    ...
    const data = await res.json();
    if (!data.success || data.targetObject !== 'Maintenance_Alert__c' || !data.records?.length) {
  ...
  await test('POST /api/ai/troubleshoot provides diagnostic steps and flags safety hazards', async () => {
    const res = await fetch(`${BASE_URL}/api/ai/troubleshoot`, {
      ...
      headers: { 'Content-Type': 'application/json', 'x-mock-mode': 'true' },
      body: JSON.stringify({
        problemDescription: 'high voltage arcing and smoke coming from transformer',
        equipmentType: 'Generator'
      })
    });
    const data = await res.json();
    if (!data.success || !data.escalateToSenior || !data.diagnosisSteps?.length) {
  ...
  await test('POST /api/sync/offline-queue replays queued mutations', async () => {
    const res = await fetch(`${BASE_URL}/api/sync/offline-queue`, {
      ...
      body: JSON.stringify({
        ...
        mutations: [
          { queueId: 'Q-01', action: 'status_update', recordId: 'WO-001001', fields: { Status__c: 'In Progress' } },
          { queueId: 'Q-02', action: 'time_log', recordId: 'WO-001001', fields: { Time_Logged_Minutes__c: 60 } }
        ]
      })
    });
    const data = await res.json();
  ...
  await test('POST /mcp tools/call get_account_summary returns top 10 accounts', async () => {
    ...
    const data = await res.json();
    const content = data.result?.content?.[0]?.text || '';
    if (!content.includes('Serum BioTech Campus') || !content.includes('DLF CyberCity')) {
      throw new Error(`MCP get_account_summary failed: ${JSON.stringify(data)}`);
    }
  });
  ...
  await test('POST /mcp tools/call execute_natural_language_query resolves prompt', async () => {
    ...
    if (!content.includes('soqlGenerated') || !content.includes('aiSummary')) {
      throw new Error(`MCP execute_natural_language_query failed: ${JSON.stringify(data)}`);
    }
  });
  ...
  if (failed > 0) {
    ...
    console.log('✨ All Mobile Backend REST & MCP API verification tests passed successfully!');
    process.exit(0);
  }
}
